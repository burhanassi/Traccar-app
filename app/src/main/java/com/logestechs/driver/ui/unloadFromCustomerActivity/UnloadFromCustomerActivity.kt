package com.logestechs.driver.ui.unloadFromCustomerActivity
...
class UnloadFromCustomerActivity : LogesTechsActivity(), View.OnClickListener,
    InsertBarcodeDialogListener {
    private lateinit var binding: ActivityUnloadFromCustomerBinding
    ...
    private fun initRecycler() {
        ...
        binding.rvScannedBarcodes.apply {
            layoutManager = LinearLayoutManager(this@UnloadFromCustomerActivity)
    ...
    private fun initialiseDetectorsAndSources() {
        ...
        binding.surfaceView.holder?.addCallback(object : SurfaceHolder.Callback {
            ...
            override fun surfaceCreated(holder: SurfaceHolder) {
                try {
                    if (Helper.isCameraPermissionNeeded(this@UnloadFromCustomerActivity)) {
                        Helper.showAndRequestCameraDialog(this@UnloadFromCustomerActivity)
                    } else {
                        cameraSource?.start(binding.surfaceView.holder)
                    }
                } catch (e: IOException) {
    ...
    private fun callUnloadFromCustomer(barcode: String?) {
        this.runOnUiThread {
            showWaitDialog()
        }
        if (Helper.isInternetAvailable(super.getContext())) {
            GlobalScope.launch(Dispatchers.IO) {
                try {
                    val response = ApiAdapter.apiClient.unloadPackageFromCustomer(
                        barcode
                    )
